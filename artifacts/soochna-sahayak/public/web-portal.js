...
let state = {
  ...
  karmachari: Array.from({ length: 5 }, () => ({
    naam: "",
    pad: "",
    bank_khataa: "",
    bank_naam: "",
...
function renderKaryaTable() {
  const pcts = getPercentArr(),
    rashis = getRashiArr(),
    total = getTotalRashi(),
    perf = getPerformancePct();
...
function renderPreview() {
  ...
  // Header info
  document.getElementById("doc-ahwc").textContent =
    state.ahwcName || "________________________";
  document.getElementById("doc-jila").textContent =
    state.jila || "____________";
  document.getElementById("doc-maah").textContent =
    `${state.maah} ${state.varsh}`;
  document.getElementById("doc-sig-ahwc").textContent =
    state.ahwcName || "______________";
  document.getElementById("doc-certify-maah").textContent =
    `${state.maah} ${state.varsh}`;

  // Table 1
  const t1rows = state.rows
    .map(
      (row, i) => `
    <tr>
...
/* =========================================================
   HEADER INPUTS
   ========================================================= */
document.getElementById("ahwc-name").addEventListener("input", (e) => {
  state.ahwcName = e.target.value;
  renderPreview();
});
document.getElementById("jila").addEventListener("input", (e) => {
  state.jila = e.target.value;
  renderPreview();
});
document.getElementById("maah").addEventListener("change", (e) => {
  state.maah = e.target.value;
  renderPreview();
});
document.getElementById("varsh").addEventListener("change", (e) => {
  state.varsh = e.target.value;
  renderPreview();
});

/* ADD ROW */
document.getElementById("add-karma-btn").addEventListener("click", () => {
  state.karmachari.push({
    naam: "",
    pad: "",
    bank_khataa: "",
    bank_naam: "",
...
document.getElementById("btn-pdf").addEventListener("click", async () => {
  const ahwc = state.ahwcName.trim(),
    jila = state.jila.trim();
  let errs = [];
  ...
  state.karmachari.forEach((emp, index) => {
    ...
    if (emp.mobile && !validateMobile(emp.mobile)) {
      errs.push(
        `कर्मचारी ${index + 1}: मोबाइल नंबर 10 अंकों का होना चाहिए (${emp.mobile})`,
      );
    }
    if (emp.bank_khataa && !validateBankAccount(emp.bank_khataa)) {
      errs.push(
        `कर्मचारी ${index + 1}: बैंक खाता न्यूनतम 9 अंक (${emp.bank_khataa})`,
      );
    }
  });
  ...
  if (typeof html2pdf === "undefined") {
    showToast(
      "⚠️ PDF लाइब्रेरी लोड हो रही है, कृपया 2 सेकंड बाद पुनः प्रयास करें",
    );
    return;
  }
  ...
  try {
    ...
    if (pdfBtn) {
      pdfBtn.classList.add("success-pulse");
      setTimeout(function () {
        pdfBtn.classList.remove("success-pulse");
      }, 600);
    }
...
(function () {
  ...
  var _todayStr =
    _td.getFullYear() +
    "-" +
    ("0" + (_td.getMonth() + 1)).slice(-2) +
    "-" +
    ("0" + _td.getDate()).slice(-2);
  ...
  function getDates() {
    ...
    var arr = [],
      curr = new Date(state.from + "T00:00:00");
  ...
  function renderDoc() {
    ...
    html +=
      '<div class="att4-office">कार्यालय राजकीय <strong>' +
      esc(state.officeName) +
      "</strong></div>";
    ...
    if (dates.length > 0) {
      html +=
        '<div class="att4-period">उपस्थिति अवधि <strong>' +
        fmtD(state.from) +
        "</strong> से <strong>" +
        fmtD(state.to) +
        "</strong> तक</div>";
    ...
    if (dates.length > 0) {
      html +=
        '<div style="overflow-x:hidden"><table class="att4-tbl" cellpadding="0" cellspacing="0">';

      /* Month header row */
      html += "<thead><tr>";
      html += '<th rowspan="2" class="att4-th att4-th-no">क्र.सं.</th>';
      html +=
        '<th rowspan="2" class="att4-th att4-th-name">नाम कार्मिक<br>मय पद</th>';
      groups.forEach(function (g) {
        html +=
          '<th colspan="' +
          g.dates.length +
          '" class="att4-th att4-th-month">' +
          esc(g.label) +
          "</th>";
      });
      html +=
        '<th rowspan="2" class="att4-th att4-th-sum"><span class="att4-th-sum-inner">उपस्थित</span></th>';
      html +=
        '<th rowspan="2" class="att4-th att4-th-sum"><span class="att4-th-sum-inner">CL</span></th>';
      html +=
        '<th rowspan="2" class="att4-th att4-th-sum"><span class="att4-th-sum-inner">अवकाश</span></th>';
      ...
      state.staff.forEach(function (s, idx) {
        ...
        dates.forEach(function (d) {
          ...
          dayCells +=
            ...
            '<select class="att4-sel no-print" data-idx="' +
            idx +
            '" data-ds="' +
            ds +
            '">' +
            opts +
            "</select>" +
            '<span class="att4-vspan print-only">' +
            esc(val) +
            "</span>" +
            "</td>";
        });
        ...
        html +=
          '<td class="att4-td att4-td-name">' +
          '<div class="att4-name-wrap">' +
          '<input class="att4-ninp no-print" data-idx="' +
          idx +
          '" data-field="name" value="' +
          esc(s.name) +
          '" placeholder="नाम" />' +
          '<input class="att4-dinp no-print" data-idx="' +
          idx +
          '" data-field="desig" value="' +
          esc(s.desig || "") +
          '" placeholder="पदनाम" />' +
          '<div class="att4-name-print print-only"><div class="att4-np">' +
          esc(s.name) +
          "</div>" +
          (s.desig ? '<div class="att4-dp">(' + esc(s.desig) + ")</div>" : "") +
          "</div>" +
          "</div></td>";
        html += dayCells;
        html +=
          '<td class="att4-td att4-td-sum" id="att4-p-' +
          idx +
          '">' +
          presentCount +
          "</td>";
        html +=
          '<td class="att4-td att4-td-sum" id="att4-cl-' +
          idx +
          '">' +
          clCount +
          "</td>";
        html +=
          '<td class="att4-td att4-td-sum" id="att4-lv-' +
          idx +
          '">' +
          leaveCount +
          "</td>";
        html +=
          '<td class="att4-td att4-td-del no-print">' +
          '<button class="att4-delbtn" data-idx="' +
          idx +
          '"' +
          (state.staff.length <= 1 ? " disabled" : "") +
          ">\u2715</button>" +
          "</td>";
        html += "</tr>";
      });
      ...
      html +=
        '<div class="att4-empty">उपस्थिति अवधि चुनें — तालिका स्वतः बनेगी</div>';
    }
    ...
    /* ── Certification ── */
    html +=
      '<div class="att4-certify">प्रमाणित किया जाता है कि उपस्थिति पत्रक का मिलान उपस्थिति पंजिका से कर लिया गया है, साथ ही कोई भी कार्मिक बिना सक्षम स्तर से अवकाश स्वीकृत कराए उपस्थिति पत्रक में उल्लिखित अवधि के दौरान अनुपस्थित नहीं रहा है।</div>';
    ...
    html +=
      ...
      '<div class="att4-sig-office">' +
      esc(state.officeName || "कार्यालय") +
      "</div>" +
      "</div>" +
      "</div>" +
      "</div>";
  ...
  function validatePeriod(from, to) {
    ...
    if (diff < 1) {
      if (typeof showToast === "function")
        showToast(
          "\u26a0\ufe0f समाप्ति तिथि प्रारंभ से पहले नहीं हो सकती",
          3000,
        );
      return false;
    }
    if (diff > 31) {
      if (typeof showToast === "function")
        showToast("\u26a0\ufe0f अधिकतम 31 दिन की अवधि चुनें", 3000);
      return false;
    }
    return true;
  }
  ...
  bind("att-office", "input", function () {
    state.officeName = this.value;
    renderDoc();
  });
  bind("att-kramank", "input", function () {
    state.kramank = this.value;
    renderDoc();
  });
  bind("att-codeno", "input", function () {
    state.codeNo = this.value;
    renderDoc();
  });
  bind("att-date", "change", function () {
    state.date = this.value;
    renderDoc();
  });
  bind("att-note", "input", function () {
    state.note = this.value;
    renderDoc();
  });

  bind("att-from", "change", function () {
    if (!validatePeriod(this.value, state.to)) {
      this.value = state.from;
      return;
    }
  ...
  bind("att-to", "change", function () {
    if (!validatePeriod(state.from, this.value)) {
      this.value = state.to;
      return;
    }
    state.to = this.value;
  ...
  /* Toggle between edit view and PDF-identical preview */
  var _previewOn = false;
  bind("att-btn-preview", "click", function () {
    _previewOn = !_previewOn;
    var doc = document.getElementById("att-doc");
    var btn = document.getElementById("att-btn-preview");
    var formWrap = document.querySelector(".att4-form-wrap");
    var banner = document.getElementById("att-preview-banner");
    if (!doc) return;
    if (_previewOn) {
      doc.classList.add("att4-pdf-mode");
      if (btn) { btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> संपादन'; }
      if (formWrap) formWrap.style.display = "none";
      if (banner) { banner.style.display = "flex"; }
      doc.scrollIntoView({ behavior: "smooth", block: "start" });
    } else {
      doc.classList.remove("att4-pdf-mode");
      if (btn) { btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> Preview'; }
      if (formWrap) formWrap.style.display = "";
      if (banner) { banner.style.display = "none"; }
    }
  });

  bind("att-btn-print", "click", function () {
    window.print();
  });
  ...
  bind("att-btn-pdf", "click", function () {
    if (!state.officeName.trim()) {
      if (typeof showToast === "function")
        showToast("\u26a0\ufe0f कार्यालय नाम आवश्यक है", 3000);
      return;
    }
    if (!state.from || !state.to) {
      if (typeof showToast === "function")
        showToast("\u26a0\ufe0f उपस्थिति अवधि आवश्यक है", 3000);
      return;
    }
    if (typeof html2canvas === "undefined" || !window.jspdf) {
      if (typeof showToast === "function")
        showToast("\u26a0\ufe0f PDF लाइब्रेरी लोड हो रही है...", 3000);
      return;
    }

    /* Exit preview-toggle mode before capture so we manage pdf-mode ourselves */
    if (_previewOn) {
      _previewOn = false;
      var _btn2 = document.getElementById("att-btn-preview");
      if (_btn2) { _btn2.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg> Preview'; }
      var _fw2 = document.querySelector(".att4-form-wrap");
      if (_fw2) _fw2.style.display = "";
      var _bn2 = document.getElementById("att-preview-banner");
      ...
      requestAnimationFrame(function () {
        document.fonts.ready
          ...
          .catch(function (err) {
            ...
            if (overlay) overlay.classList.remove("active");
            if (typeof showToast === "function")
              showToast("\u274c PDF export \u0935\u093f\u092b\u0932", 4000);
          });
      });
    });
  });
...
(function () {
  ...
  window.soundFX = {
    ...
    success: function () {
      ...
      setTimeout(function () {
        playTone(659, 0.15, 0.15, "sine");
      }, 80);
      setTimeout(function () {
        playTone(784, 0.18, 0.12, "sine");
      }, 170);
    },
    error: function () {
      playTone(300, 0.15, 0.12, "triangle");
      setTimeout(function () {
        playTone(260, 0.2, 0.1, "triangle");
      }, 120);
    },
    toggle: function () {
      playTone(600, 0.05, 0.08, "sine");
    },
  };
  ...
  document.addEventListener("click", function (e) {
    var target = e.target.closest(
      ".home-card,.btn-premium,.btn-pdf,.add-row-btn,.back-btn,.rm-btn,.sound-toggle",
    );
    if (!target) return;
...
function exportPLPToExcel() {
  ...
  var filename =
    "PLP_Report_" +
    (state.ahwcName || "AHWC").replace(/\s+/g, "_") +
    "_" +
    state.maah +
    "_" +
    state.varsh +
...
function exportAttToExcel() {
  ...
  if (!attState.from || !attState.to) {
    showToast("\u26a0\ufe0f उपस्थिति अवधि चुनें", 3000);
    return;
  }
  ...
  var _dates_xl = [],
    _curr_xl = new Date(fromDate + "T00:00:00"),
    _end_xl = new Date(toDate + "T00:00:00");
  while (_curr_xl <= _end_xl && _dates_xl.length < 31) {
    _dates_xl.push(new Date(_curr_xl));
    _curr_xl.setDate(_curr_xl.getDate() + 1);
  }
  var dates = _dates_xl;
  ...
  var monthGroupRow = [
    "अवधि - " + fmtDate(fromDate) + " से " + fmtDate(toDate) + " तक",
    "",
  ];
  var mgCounts = {};
  var mgOrder = [];
  dates.forEach(function (d) {
    ...
    if (!mgCounts[mk]) {
      ...
      mgOrder.push({
        key: mk,
        label: "माह - " + MONTHS_XL[d.getMonth()] + " " + d.getFullYear(),
      });
    }
    mgCounts[mk]++;
  });
  ...
  var wsData = [
    ...
    ["क्रमांक - उपस्थिति / " + kramankVal]
      .concat(Array(totalCols - 2).fill(""))
      .concat(["दिनांक " + fmtDate(dateVal)]),
    ["उपस्थिति पत्रक"],
    ["उपस्थिति अवधि " + fmtDate(fromDate) + " से " + fmtDate(toDate) + " तक"],
    monthGroupRow,
    ["क्र.सं.", "नाम कार्मिक मय पद"]
      .concat(dateHeaders)
      .concat([
  ...
  attState.staff.forEach(function (s, idx) {
    ...
    wsData.push(
      [idx + 1, s.name || ""]
        .concat(dayVals)
        .concat([currentPeriodCL, prevCL, currentPeriodCL + prevCL]),
    );
  });
  ...
  wsData.push([]);
  var sigRow = Array(totalCols - 1).fill("");
  sigRow.push("हस्ताक्षर प्रभारी");
  wsData.push(sigRow);
  var offRow = Array(totalCols - 1).fill("");
  offRow.push(officeName);
  wsData.push(offRow);
...
(function restorePLPDraft() {
  try {
    ...
    if (data.ahwcName) {
      ...
      var el = document.getElementById("ahwc-name");
      if (el) el.value = data.ahwcName;
    }
    if (data.jila) {
      state.jila = data.jila;
      var el2 = document.getElementById("jila");
      if (el2) el2.value = data.jila;
    }
    if (data.maah) {
      state.maah = data.maah;
      var el3 = document.getElementById("maah");
      if (el3) el3.value = data.maah;
    }
    if (data.varsh) {
      state.varsh = data.varsh;
      var el4 = document.getElementById("varsh");
      if (el4) el4.value = data.varsh;
    }
    if (data.rows && data.rows.length === 10) state.rows = data.rows;
    if (data.karmachari && data.karmachari.length > 0)
      state.karmachari = data.karmachari;
    renderAll();
...
(function restoreAttDraft() {
  try {
    ...
    if (data.officeName) {
      ...
      var el1 = document.getElementById("att-office");
      if (el1) el1.value = data.officeName;
    }
    if (data.kramank) {
      attState.kramank = data.kramank;
      var el2 = document.getElementById("att-kramank");
      if (el2) el2.value = data.kramank;
    }
    if (data.codeNo) {
      attState.codeNo = data.codeNo;
      var elc = document.getElementById("att-codeno");
      if (elc) elc.value = data.codeNo;
    }
    if (data.date) {
      attState.date = data.date;
      var el3 = document.getElementById("att-date");
      if (el3) el3.value = data.date;
    }
    if (data.from) {
      attState.from = data.from;
      var el4 = document.getElementById("att-from");
      if (el4) el4.value = data.from;
    }
    if (data.to) {
      attState.to = data.to;
      var el5 = document.getElementById("att-to");
      if (el5) el5.value = data.to;
    }
    if (data.note) {
      attState.note = data.note;
      var el6 = document.getElementById("att-note");
      if (el6) el6.value = data.note;
    }
    if (data.staffRows && data.staffRows.length > 0) {
      attState.staff = data.staffRows.map(function (s) {
        return {
          name: s.name || "",
          desig: s.desig || "",
          prevCL: s.prevCL || s.prevLeaves || 0,
          days: s.days || {},
        };
      });
    }
...
function deleteHistoryEntry(id) {
  try {
    var history = getReportHistory().filter(function (e) {
      return e.id !== id;
    });
    localStorage.setItem(HISTORY_KEY, JSON.stringify(history));
    renderHistorySection();
...
function restoreFromHistory(id) {
  var history = getReportHistory();
  var entry = history.find(function (e) {
    return e.id === id;
  });
  ...
  try {
    ...
    if (entry.type === "plp") {
      ...
      setTimeout(function () {
        if (
          ...
          var d = data;
          if (d.centerName) {
            window._yogState.centerName = d.centerName;
            var el = document.getElementById("yog-center-name");
            if (el) el.value = d.centerName;
          }
          if (d.kramank) {
            window._yogState.kramank = d.kramank;
            var el2 = document.getElementById("yog-kramank");
            if (el2) el2.value = d.kramank;
          }
          if (d.date) {
            window._yogState.date = d.date;
            var el3 = document.getElementById("yog-date");
            if (el3) el3.value = d.date;
          }
          if (d.monthVal) {
            window._yogState.monthVal = d.monthVal;
            var el4 = document.getElementById("yog-month");
            if (el4) el4.value = d.monthVal;
          }
    ...
    showToast("⚠️ रिपोर्ट लोड नहीं हो सकी");
  }
}
...
function renderHistorySection() {
  ...
  var items = history
    .map(function (entry) {
      ...
      var dateStr = date.toLocaleDateString("hi-IN", {
        day: "2-digit",
        month: "short",
        year: "numeric",
      });
      var icon =
        entry.type === "plp" ? "📊" : entry.type === "yog" ? "🧘" : "📋";
      return [
        ...
        '    <button class="history-restore-btn" onclick="restoreFromHistory(\'' +
          entry.id +
          "')\">पुनः लोड</button>",
        '    <button class="history-delete-btn" onclick="deleteHistoryEntry(\'' +
          entry.id +
...
if (window.innerWidth < 768 && !sessionStorage.getItem("btn_hint_shown")) {
  ...
  setTimeout(function () {
    showToast("👇 नीचे बटन उपलब्ध हैं", 2500);
  }, 800);
}
...
(function () {
  ...
  function getSelectedMonthMeta() {
    ...
    if (!sel || !sel.value)
      return { idx: 0, year: new Date().getFullYear(), label: "", days: 31 };
  ...
  var MALE_CAP = 8000;
  var FEMALE_CAP = 5000;
  var RATE_PER_HOUR = 250;
  var MAX_IEC_MALE = 2;
  var CERT_TEXT = "प्रमाणित किया जाता है कि उपर्युक्त टेबल में उल्लेखित दिवसों में योग प्रशिक्षक द्वारा प्रतिदिन एक घंटे से अधिक कार्य संपादित किया गया एवं उनके कार्य से संतुष्ट हूँ। कॉलम संख्या 9 में अंकित राशि के भुगतान की अनुशंसा की जाती है।";
  ...
  var yogState = {
    ...
    date: "",
    monthVal: 0,
    rows: [{ naam: "", gender: "पुरुष", hoursYog: "", hoursIEC: "" }],
  };
  ...
  function calcYogRow(row) {
    ...
    var daysInMonth = meta.days;
    var hoursYog = Math.max(0, Math.floor(parseFloat(row.hoursYog) || 0));
    hoursYog = Math.min(hoursYog, daysInMonth);
    var hoursIEC = 0;
    if (!isFemale) {
      hoursIEC = Math.max(0, Math.floor(parseFloat(row.hoursIEC) || 0));
      hoursIEC = Math.min(hoursIEC, MAX_IEC_MALE);
    }
    var totalHours = hoursYog + hoursIEC;
    var rawPayment = totalHours * RATE_PER_HOUR;
    var cap = isFemale ? FEMALE_CAP : MALE_CAP;
    var isCapped = rawPayment > cap;
    var payment = isCapped ? cap : rawPayment;
    return {
  ...
  function renderYogTable() {
    ...
    yogState.rows.forEach(function (row, idx) {
      ...
      var col5Raw = row.hoursYog === "" ? "" : String(Math.floor(parseFloat(row.hoursYog) || 0));
      var col5Err = col5Raw !== "" && (parseInt(col5Raw, 10) < 0 || parseInt(col5Raw, 10) > meta.days);
      var col6Raw = row.hoursIEC === "" ? "" : String(Math.floor(parseFloat(row.hoursIEC) || 0));
      var col6Err = !calc.isFemale && col6Raw !== "" && parseInt(col6Raw, 10) > MAX_IEC_MALE;

      html += "<tr>";
      // Col 1
      html += '<td style="text-align:center;font-weight:600">' + (idx + 1) + "</td>";
      // Col 2: Name
      html += '<td><input class="form-input-base yog-inp" style="min-width:120px;font-size:12px;text-align:left" ' +
      ...
      html += '<td><select class="form-input-base yog-sel" style="font-size:12px" ' +
        ...
        "</select></td>";
      // Col 4: Read-only auto — days in selected month
      html += '<td style="text-align:center;background:#f5f5f5;font-size:12px;padding:6px 4px;color:var(--text-secondary);font-weight:600">' +
        meta.days + "</td>";
      // Col 5: integer, 0 – meta.days
      html += '<td><input class="form-input-base yog-inp" style="font-size:12px;text-align:center;' +
        (col5Err ? "border-color:#dc2626;" : "") + '" ' +
        'type="number" min="0" max="' + meta.days + '" step="1" ' +
        'data-idx="' + idx + '" data-field="hoursYog" ' +
        'value="' + col5Raw + '" placeholder="0">' +
        (col5Err ? '<div style="color:#dc2626;font-size:9px;line-height:1.2;margin-top:1px">0–' + meta.days + ' के बीच होना चाहिए</div>' : "") +
        "</td>";
      // Col 6: N/A (female) or integer 0-2 (male)
      if (calc.isFemale) {
        html += '<td style="text-align:center;background:#e0e0e0;color:#888;font-style:italic;font-size:12px;pointer-events:none;cursor:not-allowed">N/A</td>';
      } else {
        html += '<td><input class="form-input-base yog-inp" style="font-size:12px;text-align:center;' +
          (col6Err ? "border-color:#dc2626;" : "") + '" ' +
          'type="number" min="0" max="2" step="1" ' +
          'data-idx="' + idx + '" data-field="hoursIEC" ' +
          'value="' + col6Raw + '" placeholder="0">' +
          (col6Err ? '<div style="color:#dc2626;font-size:9px;line-height:1.2;margin-top:1px">IEC घंटे अधिकतम 2 हो सकते हैं</div>' : "") +
          "</td>";
      }
      // Col 7: read-only computed total
      html += '<td style="text-align:center;background:#f5f5f5;font-size:12px;padding:6px 4px;font-weight:600">' +
        calc.totalHours + "</td>";
      // Col 8: fixed rate — read-only
      html += '<td style="text-align:center;background:#f5f5f5;font-size:12px;padding:6px 4px">₹250/-</td>';
      // Col 9: computed payment — amber with indicator if capped
      var capStyle = calc.isCapped
        ? "color:#d97706;font-weight:700"
        : "font-weight:700;color:var(--accent-primary)";
      var capSuffix = calc.isCapped
        ? ' <span style="font-size:9px;color:#d97706">(सीमित)</span>'
        : "";
      html += '<td style="text-align:right;background:#f5f5f5;font-size:12px;padding:6px 4px">' +
        '<span style="' + capStyle + '">₹' + calc.payment.toLocaleString("en-IN") + '/-</span>' + capSuffix + "</td>";
      // Remove button
      html += '<td class="no-print"><button class="rm-btn" onclick="window._yogRemoveRow(' + idx + ')" ' +
        (yogState.rows.length <= 1 ? "disabled" : "") + ">✕</button></td>";
      html += "</tr>";
    });

    tbody.innerHTML = html;
    document.getElementById("yog-add-row-btn") && (document.getElementById("yog-add-row-btn").style.display = "");
    ...
    tbody.querySelectorAll(".yog-inp").forEach(function (inp) {
      inp.addEventListener("input", function () {
        ...
        if (trs[idx]) {
          ...
          // td index: 0=क्र, 1=नाम, 2=gender, 3=col4, 4=col5, 5=col6, 6=col7, 7=col8, 8=col9, 9=del
          if (tds[6]) tds[6].textContent = calc.totalHours;
          if (tds[8]) {
            var cs = calc.isCapped ? "color:#d97706;font-weight:700" : "font-weight:700;color:var(--accent-primary)";
            var csuf = calc.isCapped ? ' <span style="font-size:9px;color:#d97706">(सीमित)</span>' : "";
            tds[8].innerHTML = '<span style="' + cs + '">₹' + calc.payment.toLocaleString("en-IN") + '/-</span>' + csuf;
          }
        }
        if (!window._debouncedRenderYogDoc)
          window._debouncedRenderYogDoc = debounce(renderYogDoc, 200);
  ...
  function renderYogDoc() {
    ...
    var CR = "border:1px solid #000;padding:3px 3px;text-align:right;vertical-align:middle;font-size:6.5pt;font-weight:700;";
    ...
    var rows = yogState.rows.map(function (row, idx) {
      var calc = calcYogRow(row);
      var col6 = calc.isFemale
        ? '<td style="' + CN + '">N/A</td>'
        : '<td style="' + C + '">' + (calc.hoursIEC || 0) + "</td>";
      var payStr = "₹" + calc.payment.toLocaleString("en-IN") + "/-";
      return "<tr>" +
        '<td style="' + C + '">' + (idx + 1) + "</td>" +
        '<td style="' + CL + '">' + esc(row.naam || "") + "</td>" +
        '<td style="' + C + '">' + row.gender + " योग प्रशिक्षक</td>" +
        '<td style="' + C + '">' + meta.days + "</td>" +
        '<td style="' + C + '">' + (calc.hoursYog || 0) + "</td>" +
        col6 +
        '<td style="' + C + 'font-weight:700;">' + (calc.totalHours || 0) + "</td>" +
        '<td style="' + C + '">₹250/-</td>' +
        '<td style="' + CR + '">' + payStr + "</td>" +
        "</tr>";
    }).join("");
  ...
  function attachYogHeaderListeners() {
    ...
    if (centerEl)
      centerEl.addEventListener("input", function () {
  ...
  function attachYogAddRow() {
    ...
    if (!btn) return;
    btn.addEventListener("click", function () {
      yogState.rows.push({ naam: "", gender: "पुरुष", hoursYog: "", hoursIEC: "" });
      renderYogTable();
      renderYogDoc();
    });
  }
  ...
  function exportYogPDF() {
    ...
    // Pre-PDF validation gate
    var errors = [];
    if (!yogState.centerName || !yogState.centerName.trim()) errors.push("केंद्र का नाम भरें");
    if (!yogState.kramank || !yogState.kramank.trim()) errors.push("क्रमांक भरें");
    if (!yogState.date) errors.push("दिनांक भरें");
    if (!yogState.rows.length) errors.push("कम से कम एक योग प्रशिक्षक जोड़ें");
    yogState.rows.forEach(function (row, i) {
      if (!row.naam || !row.naam.trim()) errors.push("पंक्ति " + (i + 1) + ": प्रशिक्षक का नाम भरें");
    });
    var errBox = document.getElementById("yog-err-box");
    if (errors.length > 0) {
      if (errBox) {
  ...
  function printYogDoc() {
    ...
    win.document.write(
      "<html><head><title>योग प्रशिक्षक उपस्थिति पत्रक</title>",
    );
    win.document.write(
      "<style>" +
  ...
  function exportYogExcel() {
    ...
    var wsData = [
      ...
      ["क्रमांक: " + (yogState.kramank || ""), "", "", "", "", "", "", "", "दिनांक: " + (yogState.date || "")],
      ["योग प्रशिक्षक मानदेय उपस्थिति पत्रक", "", "", "", "", "", "", "", "माह: " + meta.label],
      [],
    ...
    yogState.rows.forEach(function (row, idx) {
      ...
      wsData.push([
        idx + 1,
        row.naam || "",
        row.gender + " योग प्रशिक्षक",
        meta.days,
        calc.hoursYog,
        calc.isFemale ? "N/A" : calc.hoursIEC,
        calc.totalHours,
        "₹250/-",
        "₹" + calc.payment.toLocaleString("en-IN") + "/-",
      ]);
    });
    wsData.push([]);
    wsData.push([CERT_TEXT]);
    var ws = XLSX.utils.aoa_to_sheet(wsData);
    ws["!cols"] = [{ wch: 6 }, { wch: 28 }, { wch: 20 }, { wch: 14 }, { wch: 18 }, { wch: 18 }, { wch: 14 }, { wch: 14 }, { wch: 20 }];
